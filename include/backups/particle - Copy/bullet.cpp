#include "bullet.h"

namespace solstice {
    Bullet::Bullet() {
        mAlive = false;
        mShot = false;
        mTransfersVelocity = false;
        mTransfersPosition = false;
        mEmitsOnImpact = false;
        mMaxBounces = 0;
        mBounces = 0;
        mElasticityX = 0.0f;
        mElasticityY = 0.0f;
        mLocksOn = false;
        mTargetFound = false;
        mAdjustsAngle = false;
        mLockOnVel = 0.0f;
        mUpdateFunc = nullptr;
        mCollisionFunc = nullptr;
        mRotatesSprite = false;
        mCollisionFound = false;
        mLockOnStartVel = 0;
        mAnimations.resize(2);
        mBlastForce = 0.0f;
        mDirectsImpact = false;
        mSpriteAngle = 0.0;
        mTrackingSpeed = 1.0f;
        mType = BTYPE_PROJECTILE;
        mWaterStops = false;
        mCollisionType = 0;
        mBouncesTowards = false;
        mAutoDeathDur = -1;
        mCreatesSubBullets = false;
        mCreateSubState = -1;
    }
    void Bullet::Update(float dt) {
        if(!mUpdateFunc) {
            DefaultUpdate(dt);
        }
        else {
            mUpdateFunc(this, dt);
        }
    }
    void Bullet::DefaultUpdate(float dt) {
        if(mAlive) {
            if(mAutoDeathDur > 0 && mCurFrame >= mAutoDeathDur) {
                Destroy();
            }
            else if(mMaxBounces) {
                if(mCollisionFound) {
                    if(mCollisionType & mCollidesWidth) {
                        mBounces++;
                    }
                }
                if(mBounces > mMaxBounces) {
                    Destroy();
                }
            }
            if(!mCollisionFound && mTargetFound && mAdjustsAngle) {
                mTargetFound = false;
            }
            if(mTargetFound && glm::length(mVelocity) < glm::length(mMaxSpeed)) {
                mLockOnVel+= mLockOnAccel;
                mVelocity = glm::vec3(mLockOnDir.x, mLockOnDir.y, 0.0f) * mLockOnVel;
            }
            if(mTargetFound) {
                AimTowards(mLockOnPoint);
            }
            Particle::Update(dt);
            mAcceleration = glm::vec3(0.0f);
            mHitBox.AddXY(mVelocity.x, mVelocity.y);
            mHitBox.SetAngle(mAngle);
            mPosition.x = mHitBox.GetPosition().x;
            mPosition.y = mHitBox.GetPosition().y;
            Vector2f center = mHitBox.GetCenter();
            if(mTransfersPosition) {
                mEmitter.SetPosition(glm::vec3(center.x, center.y, 0.0f));
            }
            if(mTransfersVelocity) {
                mEmitter.AddVelocity(glm::vec3(mVelocity.x, mVelocity.y, 0.0));
            }
            //mEmitter.Update(dt);
            if(mType == BTYPE_LASER) {
                mEmitter.SetInfluenceAngle(Vector2f(cos(mAngle), sin(mAngle)));
            }
            mEmitter.Update(dt, mPosition);
            mLockOnRadius.SetPosition(center);
        }
        else if(mEmitsOnImpact && mShot) {
            if(mType == BTYPE_PROJECTILE) {
                Vector2f center = mHitBox.GetCenter();
                if(mTransfersPosition) {
                    mImpactEmitter.SetPosition(glm::vec3(mCollisionPoint.x, mCollisionPoint.y, 0.0f));
                }
                if(mTransfersVelocity) {
                    mImpactEmitter.AddVelocity(glm::vec3(mVelocity.x, mVelocity.y, 0.0));
                }
                mImpactEmitter.Update(dt);
            }
        }
        if(mType == BTYPE_LASER && mCollisionFound) {
            glm::vec3 colPoint(mCollisionPoint.x, mCollisionPoint.y, 0.0f);
            mImpactEmitter.SetSpawnPositions(colPoint);
            mImpactEmitter.Update(dt);
            if(mCollisionFound) {
                //mImpactEmitter.SetPosition(colPoint);
            }
        }
        mBlastRadius.SetPosition(mCollisionPoint);
        mCollisionFound = false;
        if(mShot && mRotatesSprite) {
            RotateSprite();
        }
        UpdateAnimation(dt);
        mCollisionFound = false;
        mCollisionType = 0;
    }
    void Bullet::RotateSprite() {
        if(mType == BTYPE_PROJECTILE && mRotatesSprite) {
            mSpriteAngle = atan2(mVelocity.y, mVelocity.x);
        }
    }
    void Bullet::UpdateAnimation(float dt) {
        if(mAnimations.empty()) {
            return;
        }
        if(mAlive) {
            for(size_t i = 0; i < mAnimations.size() - 1; i++) {
                mAnimations[i].SetPosition(GetHitBoxPos());
                mAnimations[i].SetAngle(mSpriteAngle);
                mAnimations[i].SetPosition(GetHitBoxPos());
                mAnimations[i].Update(dt);
            }
            mAnimations.back().SetPosition(GetHitBoxPos());
        }
        else if(mShot && mAnimations.size() >= 2 && !mAnimations.back().Finished()) {
            mAnimations.back().SetAngle(mSpriteAngle);
            mAnimations.back().SetPosition(GetHitBoxPos());
            mAnimations.back().Update(dt);
        }
    }
    bool Bullet::CheckAndResolveCollision(BackgroundTile2& ground) {
        if(ground.GetType() == TILE_PLATFORM) {
            return false;
        }
        if(mAlive && (mCollidesWidth & PDESTROY_WALL) || (mCollidesWidth & PDESTROY_GROUND)) {
            Collision c = CheckCollision(&mHitBox, &ground.GetHitBox());
            if(c) {
                if(fabs(c.normal.x) >= 0.25) {
                    mCollisionType|= PDESTROY_WALL;
                }
                if(fabs(c.normal.y) >= 0.25) {
                    mCollisionType|= PDESTROY_GROUND;
                }
                if((mDestroyedBy & mCollisionType)) {
                    if(mType == BTYPE_PROJECTILE) {
                        Destroy(Vector2f(mVelocity.x, mVelocity.y));
                    }
                    SetCollisionPoint(c.GetContactPoint(mHitBox, ground.GetHitBox()));
                }
                else {
                    ResolveCollision(c, ground.GetFriction());
                    mCollisionFound = true;
                }
                return true;
            }
        }
        return false;
    }
    bool Bullet::CheckAndResolveCollision(RigidBody* b) {
        if(!b) {
            return false;
        }
        if(mAlive && (mCollidesWidth & PDESTROY_OBJECT)) {
            Collision c = CheckCollision(&mHitBox, &b->GetHitBox());
            if(c) {
                mCollisionType|= PDESTROY_OBJECT;
                if((mDestroyedBy & mCollisionType)) {
                    if(mType == BTYPE_PROJECTILE) {
                        Destroy(Vector2f(mVelocity.x, mVelocity.y));
                    }
                    SetCollisionPoint(c.GetContactPoint(mHitBox, b->GetHitBox()));
                }
                if(mType == BTYPE_PROJECTILE) {
                    Vector2f impact = mImpactForce;
                    if(mVelocity.x < 0) {
                        impact.x = -impact.x;
                    }
                    if(mVelocity.y < 0) {
                        impact.y = -impact.y;
                    }
                    b->ApplyImpulse(impact);
                    mCollisionFound = true;
                    ResolveCollision(c, b->GetFriction());
                }
                return true;
            }
        }
        else if(mBlastForce > 0.0 && mShot && mAnimations.back().GetCurFrameNum() <= 1) {
            Collision c = CircleRectCollision(mBlastRadius, b->GetHitBox());
            if(c) {
                Vector2f blastForce = ResolveBlast(b->GetHitBox().GetCenter());
                if(!b->GetNoTorque()) {
                    float angle = blastForce.Cross(Vector2f(mVelocity.x, mVelocity.y));
                    b->AddRotVelocity(angle * b->GetInertia() * 10.0f);
                }
                b->ApplyImpulse(blastForce);
                return true;
            }
        }
        if(mLocksOn && (!mTargetFound || (mTargetFound && mAdjustsAngle))) {
            Collision c = CircleRectCollision(mLockOnRadius, b->GetHitBox());
            if(c && !mTargetFound) {
                AimTowards(b->GetHitBox().GetCenter());
                return true;
            }
        }
        return false;
    }
    bool Bullet::CheckAndResolveCollision(WaterBody& b) {
        return false;
    }
    void Bullet::DefaultResolveCollision(Collision& c, float friction) {
        if(c && mAlive) {
            mHitBox.AddXY(c.mtv);
            Vector2f vel(mVelocity.x, mVelocity.y);
            Vector2f normalVel = c.normal.Dot(vel) * c.normal;
            Vector2f tangentVel = vel - normalVel;
            normalVel.x*= -(1 + mElasticityX);
            normalVel.y*= -(1 + mElasticityY);
            if(mTargetFound && mBouncesTowards) {
                Vector2f dir = GetHitBoxPos() - mLockOnPoint;
                float angle = atan2(dir.y, dir.x);
                normalVel = Vector2f(cos(angle), sin(angle)) * normalVel.Magnitude();
            }
            vel = tangentVel * friction + normalVel;
            mVelocity.x+= vel.x;
            mVelocity.y+= vel.y;
            mCollisionFound = true;
        }
    }
    void Bullet::SetCollisionPoint(Vector2f point) {
        if(point.Distance(mCollisionPoint) > mHitBox.GetHeight() * mHitBox.GetHeight()) {
            mCollisionPoint = point;
        }
        if(mEmitsOnImpact) {
            mImpactEmitter.SetPosition(glm::vec3(point.x, point.y, 0.0));
        }
        mCollisionFound = true;
    }
    Vector2f Bullet::ResolveBlast(Vector2f point) {
        Vector2f diff = point - mBlastRadius.GetCenter();
        float dist = diff.Length2();
        float r2 = mBlastRadius.GetRadius() * mBlastRadius.GetRadius() * 0.5;
        float attn = r2 / dist;
        Vector2f force = diff.GetUnit();
        return force * mBlastForce * attn;
    }
    void Bullet::ResolveCollision(Collision& c, float friction) {
        if(mCollisionFunc) {
            mCollisionFunc(this, c, friction);
        }
        else {
            DefaultResolveCollision(c, friction);
        }
    }
    void Bullet::AimTowards(Vector2f pos) {
        if(mType == BTYPE_PROJECTILE) {
            if(!mTargetFound || (mTargetFound && mAdjustsAngle)) {
                mLockOnPoint = pos;
                if(!mBouncesTowards) {
                    pos = pos - GetHitBoxPos();
                    float vel = mLockOnStartVel;
                    if(vel < 0.5) {
                        vel = glm::length(mVelocity);
                    }
                    if(vel > mMaxSpeed.x) {
                        vel = mMaxSpeed.x;
                    }
                    float angle = atan2(pos.y, pos.x);
                    float dirAngle = atan2(mVelocity.y, mVelocity.x);
                    angle = DeltaAngle(dirAngle, angle);
                    float angleAdjust = dirAngle + GetTrackedAngle(angle);
                    mLockOnDir = Vector2f(cos(angleAdjust), sin(angleAdjust));
                    mVelocity = glm::vec3(mLockOnDir.x, mLockOnDir.y, 0.0f) * vel;
                    mTargetFound = true;
                }
            }
        }
        else if(mType == BTYPE_LASER) {
            pos = pos - GetHitBoxPos();
            float angle = atan2(pos.y, pos.x);
            angle = GetTrackedAngle(DeltaAngle(mAngle, angle));
            mAngle = mAngle + angle;
            mSpriteAngle = mAngle + angle;
        }
    }
    float Bullet::GetTrackedAngle(float a) {
        if(mTrackingSpeed < 0) {
            if(a < mTrackingSpeed) {
                a = mTrackingSpeed;
            }
            else if(a > -mTrackingSpeed) {
                a = -mTrackingSpeed;
            }
        }
        else {
            a*= mTrackingSpeed;
        }
        return a;
    }
    void Bullet::Destroy(Vector2f force) {
        if(!mAlive) {
            return;
        }
        mAlive = false;
        if(mCreatesSubBullets) {
            mCreateSubState = 1;
        }
        if(mEmitsOnImpact) {
            if(mDirectsImpact) {
                mImpactEmitter.SetInfluenceAngle(-force);
            }
            mImpactEmitter.SetPosition(mPosition);
            mImpactEmitter.Reset();
        }

        mAnimations.back().Reset();
    }
    void Bullet::Draw(ShaderProgram& shader, TextureArray& textures) {
        if(mAlive) {
            mEmitter.Draw(shader, textures);
        }
        if((!mAlive && mEmitsOnImpact && mShot) || (mCollisionFound && mType == BTYPE_LASER)) {
            mImpactEmitter.Draw(shader, textures);
        }
    }
    void Bullet::InitEmitters() {
        mEmitter.CreateBuffers();
        mImpactEmitter.CreateBuffers();
    }
    void Bullet::Reset() {Reset(Vector2f(mInitialState.position.x, mInitialState.position.y));}
    void Bullet::Reset(Vector2f pos) {
        Particle::Reset();
        mEmitter.Reset();
        mAlive = false;
        mShot = false;
        mCreateSubState = -1;
        mTargetFound = false;
        mLockOnPoint = 0;
        mLockOnDir = 0;
        mBounces = 0;
        SetPosition(pos);
        mVelocity.x = mInitialState.velocity.x;
        mVelocity.y = mInitialState.velocity.y;
        mLockOnRadius.SetPosition(GetHitBoxPos());
        mBlastRadius.SetPosition(GetHitBoxPos());

        mTargetFound = false;
        mAnimation.Reset();
        for(auto& anim : mAnimations) {
            anim.Reset();
            anim.SetPosition(pos);
        }
        if(mRotatesSprite) {
            if(mVelocity.x < 0) {
                SetHoriFlip(true);
            }
            else {
                SetHoriFlip(false);
            }
        }
        else {
            if(mVelocity.x < 0) {
                SetVertFlip(true);
            }
            else {
                SetVertFlip(false);
            }
        }
        UpdateAnimation(0);
    }
    void Bullet::InitHitBox(Vector2f pos, float h, float w, int drawFrom) {
        mHitBox.Init(pos, h, w, (DrawPoint)drawFrom);
    }
    void Bullet::Shoot(Vector2f velocity) {
        Reset();
        float x = mInitialState.velocity.x;
        if(velocity.x < 0) {
            x = -x;
        }
        mVelocity.x = velocity.x + x;
        mVelocity.y = velocity.y + mInitialState.velocity.y;
        mAlive = true;
        mShot = true;
        if(mAdjustsAngle) {
            mAngle = atan2(mVelocity.y, mVelocity.x);
        }

    }
    void Bullet::Shoot(Vector2f velocity, Vector2f position) {
        Shoot(velocity);
        SetPosition(position);
    }
    void Bullet::ShootAngle(float angle, float vel) {
        Reset();
        mAngle = angle;
        mAngleVel = vel;
        mAlive = true;
        mShot = true;
        if(mAdjustsAngle) {
            mAngle = atan2(mVelocity.y, mVelocity.x);
        }
    }
    void Bullet::SetScript(kaguya::State& state, std::string update, std::string collision) {
        mUpdateFunc = state[update];
        mCollisionFunc = state[collision];
        state["Init"](this);
    }
    void Bullet::SetScript(kaguya::State& state, std::string package) {
        mUpdateFunc = state[package]["Update"];
        mCollisionFunc = state[package]["Collide"];
        state[package]["Init"](this);
    }
    void Bullet::SetDestroyAnim(Animation anim) {
        if(!mAnimations.empty()) {
            mAnimations.back() = anim;
        }
    }
    void Bullet::AddAnimation(Animation anim) {
        if(mAnimations.size() >= 2) {
            mAnimations.insert(mAnimations.end() - 1, anim);
        }
        else {
            mAnimations.push_back(anim);
        }
    }
    void Bullet::SetAnimation(size_t i, Animation anim) {
        if(i < mAnimations.size()) {
            mAnimations[i] = anim;
        }
    }
    void Bullet::Draw(SceneRenderer& rend) {
        if(mAlive) {
            for(size_t i = 0; i < mAnimations.size() - 1; i++) {
                mAnimations[i].SetPosition(GetHitBoxPos());
                rend.DrawQuad(mAnimations[i].GetDrawQuad());
            }
        }
        else if(mShot && !mAnimations.back().Finished()) {
            rend.DrawQuad(mAnimations.back().GetDrawQuad());
        }
    }
    bool Bullet::Finished() {
        if(mAlive) {
            return false;
        }
        if(mAnimations.back().Finished()) {
            if(!mEmitsOnImpact) {
                return true;
            }
            else {
                return mImpactEmitter.Finished();
            }
        }
        return false;
    }
    void Bullet::SetHoriFlip(bool f) {
        for(auto& anim : mAnimations) {
            anim.SetHoriFlip(f);
        }
    }
    void Bullet::SetVertFlip(bool f) {
        for(auto& anim : mAnimations) {
            anim.SetVertFlip(f);
        }
    }
}
