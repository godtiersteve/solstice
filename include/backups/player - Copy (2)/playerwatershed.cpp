...
namespace solstice {
    ...
    void WatershedPlayer::Walk() {
        ...
        if(mLeftHeld) {
            ...
            if(!mFocusMode && mGrounded && velocity.x >= walkSpeed * 0.90) {
                ...
                else if(GetCurrentAnimID() == IONA_RUNSHOOT_DFORWARD) {
                    ChangeAnimIDSpecial(IONA_SHOOT_BRAKE_DFORWARD, true, 0.0f);
                }
                else if(GetCurrentAnimID() == IONA_RUNSHOOT_UFORWARD) {
        ...
        if(((!mLeftHeld && !mRightHeld) && (GetCurrentAnimID() != IONA_HOOK || (GetCurrentAnimID() == IONA_HOOK && mGrounded)))) {
            if(accel > 0.0) {
                if(velocity.x > 0) {
                    ApplyForceX(-accel);
                }
            ...
            if(mHeldObject && (GetCurrentAnimID() < IONA_HOOK || GetCurrentAnimID() > IONA_HOOK_PULL_FINS)) {
                if(mDownHeld) {
                    ...
                    if(mVelocity.x) {
                        ...
                        if(mUpHeld) {
                            ChangeAnimIDSpecial(IONA_SHOOT_RECOIL_UP, true);
                        }
                        else if(mDownHeld) {
                            ChangeAnimIDSpecial(IONA_SHOOT_RECOIL_DOWN, true);
                        }
                        else {
                            ChangeAnimIDSpecial(IONA_SHOOT_RECOIL, true);
                        }
                        mRecoiling = true;
                    }
                    ThrowObject();
        ...
        if(mUnderwater) {
            ...
            if(mGrounded) {
                ...
                if(GetCurrentAnimID() == IONA_SHOOT_JUMP || GetCurrentAnimID() == IONA_SHOOT_BACK_JUMP ||
                   GetCurrentAnimID() == IONA_SHOOT_JUMP_DOWN|| GetCurrentAnimID() == IONA_SHOOT_JUMP_UP)
                {
                    transSpeed = 0.05f;
                }
                if(mDownHeld) {
                ...
                if(!mSurfaced && mVelocity.y >= -1.0f) {
                    ...
                    if(mVelocity.x != 0.0f) {
                        if(GetCurrentAnimID() == IONA_TURN_MERMAID) {
                            ...
                            TransitionToAnimation(IONA_MERMAID_SWIM, 0.025f, 0, 0);
        ...
        if(!UsesFocusAnims() && !mSpecialAnim || (mSpecialAnim && mCancelableSpecial)) {
            ...
            if(mVelocity.Magnitude2() > 0.0f && !mSurfacing && !mDiving) {
                ...
                if(GetCurrentAnimID() == IONA_SWIM_BRAKE && !mSpecialAnim) {
                    ...
                    TransitionToAnimation(IONA_SWIM, 0.05, 0, 11);
                }
    ...
    void WatershedPlayer::ChangeAnimID(int baseAnim, int newAnim, int& i, float& dur, int& forceFrame) {
        i = newAnim;
        if(GetCurrentAnimID() == baseAnim) {
            dur = 0.0f;
    ...
    void WatershedPlayer::ChangeAnimID(int i, float transitionDur) {
        ...
        if(mRecoiling) {
            switch(i) {
            case IONA_SHOOT_GROUND:
                ChangeAnimID(IONA_SHOOT_GROUND, IONA_SHOOT_RECOIL, i, transitionDur, forceFrame);
                break;
            case IONA_SHOOT_UP:
                ChangeAnimID(IONA_SHOOT_UP, IONA_SHOOT_RECOIL_UP, i, transitionDur, forceFrame);
                break;
            case IONA_SHOOT_DOWN:
                ChangeAnimID(IONA_SHOOT_DOWN, IONA_SHOOT_RECOIL_DOWN, i, transitionDur, forceFrame);
                break;
            default:
                break;
            }
        ...
        if(mFins) {
            ...
            case IONA_SHOOT_JUMP:
                ChangeAnimID(IONA_SHOOT_JUMP, IONA_SHOOT_SWIM_JUMP, i, transitionDur, forceFrame);
                break;
            ...
            case IONA_SHOOT_JUMP_UP:
                ...
                if(GetCurrentAnimID() == IONA_SHOOT_JUMP_UP) {
                    transitionDur = 0.0f;
                    forceFrame = GetCurrentFrameNum();
                }
                break;
            case IONA_SHOOT_RECOIL_UP:
                i = IONA_SHOOT_RECOIL_UP_FINS;
                if(GetCurrentAnimID() == IONA_SHOOT_RECOIL_UP) {
                    transitionDur = 0.0f;
                    forceFrame = GetCurrentFrameNum();
                }
                break;
            case IONA_SHOOT_RECOIL_DOWN:
                i = IONA_SHOOT_RECOIL_DOWN_FINS;
                if(GetCurrentAnimID() == IONA_SHOOT_RECOIL_DOWN) {
                    transitionDur = 0.0f;
                    forceFrame = GetCurrentFrameNum();
                }
                break;
            case IONA_SHOOT_RECOIL:
                i = IONA_SHOOT_RECOIL_FINS;
                if(GetCurrentAnimID() == IONA_SHOOT_RECOIL) {
                    transitionDur = 0.0f;
                    forceFrame = GetCurrentFrameNum();
                }
                break;
    ...
    void WatershedPlayer::GenerateAnimCache() {
        ...
        //mAnimationCache[i++] = mModel.SetAnimID("UShootFJumpA", IONA_SHOOT_JUMP_UFORWARD);
        //mAnimationCache[i++] = mModel.SetAnimID("DShootFJumpA", IONA_SHOOT_JUMP_DFORWARD);
        //mAnimationCache[i++] = mModel.SetAnimID("", IONA_SHOOT_JUMP_UBACK);
        //mAnimationCache[i++] = mModel.SetAnimID("", IONA_SHOOT_JUMP_DBACK);
        ...
        //mAnimationCache[i++] = mModel.SetAnimID("UShootFJumpB", IONA_SHOOT_JUMP_UFORWARD_FINS);
        //mAnimationCache[i++] = mModel.SetAnimID("DShootFJumpB", IONA_SHOOT_JUMP_DFORWARD_FINS);
        //mAnimationCache[i++] = mModel.SetAnimID("", IONA_SHOOT_JUMP_UBACK_FINS);
        //mAnimationCache[i++] = mModel.SetAnimID("", IONA_SHOOT_JUMP_DBACK_FINS);
