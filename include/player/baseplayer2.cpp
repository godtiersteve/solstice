...
void BasePlayer2::Init() {
    ...
    mMaxMana = 100;
    mMaxVelocity.y = 30;
    mObjectType = OBJECT_PLAYER;
    SetNoTorque(true);
    mInterpModifier = 1.0f;
}
...
void BasePlayer2::SetDirectionFlags() {
...
bool BasePlayer2::AnimationHasFlag(int flag) {
    if(mModel.Empty()) {
        return false;
    }
    else {
        return GetCurrentAnim().HasFlag(flag);
    }
}
...
void BasePlayer2::Update(double dt) {
    mOnPlatform = false;
    if(mAnimations.empty() && mModel.GetMeshes().empty()) {
        mSpecialAnim = false;
    }
...
int BasePlayer2::GetCurrentAnimID() {
    if(mUsesModel) {
        return mModel.GetCurrentAnim() ? mModel.GetCurrentAnim()->GetID() : -1;
    }
    else {
...
void BasePlayer2::ChangeAnimation(int i, float transitionDur) {
    ...
    if(mUsesModel) {
        mModel.ChangeAnimation(i, transitionDur * mInterpModifier);
    }
    else {
...
void BasePlayer2::CheckAndResolveCollision(RigidBody* b) {
    if(this == b) {
        return;
    }
    Collision c = solstice::CheckCollision(GetHitShapef(), b->GetHitShapef());
    //Collision c = solstice::CheckCollision(&GetHitBox(), &b->GetHitBox());
    if(!c || c.mtv.Length2() < 4.0) {
        if(c.normal.y > 0.25) {
            mGrounded = true;
        }
        if(fabs(c.normal.y) > 0.75) {
            mLastPos.y = GetPosition().y;
        }
    ...
    if(c && fabs(GetInvMass()) + fabs(b->GetInvMass()) > 0.001) {
        ...
        if(!mUnderwater && !b->GetUnderWater()) {
            if(mInvMass > 0.0 && b->GetInvMass() > 0.0) {
        ...
        if(b->GetIsPlatform() && c.normal.y > -0.1f) {
...
void BasePlayer2::CheckAndResolveCollision(BackgroundTile2& ground) {
    ...
    for(size_t i = 0; i < ground.GetHitBox2().Size(); i++) {
        Collision c = solstice::CheckCollision(&GetHitBox(), ground.GetHitBox2().GetHitBox(i).get());
        if(!c || c.mtv.Length2() < 2.0) {
            if(c.normal.y > 0.25) {
                SetGrounded(true);
            }
            if(fabs(c.normal.y) > 0.75) {
        ...
        if(fabs(c.mtv.x) < 2.0 && c.mtv.y > 0.1) {
            //c.normal.x = 0;
            //c.normal.y = 1;
        }
...
void BasePlayer2::ResolveCollision(Collision& c, float mass, Vector2f vel) {
    if(fabs(c.mtv.y) > 0.01 && fabs(c.mtv.x) > 0.01) {
...
void BasePlayer2::ResolveCollision(RigidBody* b, Collision& c, Vector2f vel) {
    ...
    if(b->GetGrounded() && c.normal.y >= 0.8) {
        massAy = 1.0;
        massBy = 0.0;
    }
    ...
    if(fabs(c.normal.y) > 0.1f && fabs(c.mtv.x) > 0.01) {
    ...
    if(c.normal.y > 0.25f && b1Vel.y < 0.0f) {
        SetGrounded(true);
    }
    if(c.normal.y < -0.25f && b2Vel.y < 0.0f) {
        b->SetGrounded(true);
    }
}
...
void BasePlayer2::UpdateModel(float t, Vector2f scale) {
    ...
    pos.z = mModel.GetTransform().GetTranslation().z;
...
void BasePlayer2::OnKeyUp(PlayerKey key) {
}
void BasePlayer2::OnKeyDown(PlayerKey key) {
}
void BasePlayer2::JumpReleased() {
}
void BasePlayer2::DirectionReleased(PlayerKey direction) {
}
void BasePlayer2::AttackReleased() {
}
void BasePlayer2::SpecialReleased() {
}
