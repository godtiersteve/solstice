...
protected:
    ...
    float mHealth, mMana, mMaxMana, mMaxHealth;
    ...
    virtual void SetDirectionFlags();
    void Float();
    void CheckInput();
    void CheckPlatform();
    ...
    virtual void DirectionReleased(PlayerKey direction);
    virtual void AttackReleased();
    virtual void SpecialReleased();
public:
    float mInterpModifier;
    BasePlayer2();
    ...
    void SetMana(float m)                           {mMana = m;}
    void SetMaxMana(float m)                        {mMaxMana = m;}
    ...
    bool AnimHasFlag(int flag)                      {return mModel.GetCurrentAnim() ? mModel.GetCurrentAnim()->HasFlag(flag) : false;}

    virtual void Update(std::vector<SDL_Event>& events, float tstep = 1.0f);
    virtual void Update(double dt);
    virtual void UpdateKeys(SDL_Event& event);
    virtual void UpdateKeys(vector<SDL_Event>& events);
    virtual void UpdateKeys(SDL_Event& event, SDL_GameController* controller);
    ...
    bool AnimationHasFlag(int flag);
    virtual void CheckAndResolveCollision(BackgroundTile2& ground);
    virtual void CheckAndResolveCollision(RigidBody* b);
    virtual bool CheckCollision(WaterBody& waterBody)   {return PhysicsBox2f::CheckCollision(waterBody);}
    virtual void Attack() {}
};
