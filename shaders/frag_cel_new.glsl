#version 400
subroutine vec4 fragShaderType();
subroutine uniform fragShaderType fragType;
...
subroutine(fragShaderType)
vec4 phongCel() {
	return texture(tex, vec2(fragTex.x, -fragTex.y)) * fragColor;
}
...
subroutine(fragShaderType)
vec4 normalRend() {
	return texture(tex, vec2(fragTex.x, fragTex.y)) * vec4(color, 0.75f);
}
void main() {	
	outColor = fragType();
	outNorm = vec4(norm, 1.0f);
}
